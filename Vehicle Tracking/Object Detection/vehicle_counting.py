...
while(True):
        ...
        if ret:
            ...
            for (bbox_xyxy,confidence,cls) in zip(bbox_xyxys,confidences,labels):
                ...
                ''' 
                detections keeps it in 2d array 0th index as  x1,y1,x2,y2,id for 0th frame,
                then appended again for next frame 1st index for 1st(assumption) frame.
                '''
                detections=np.vstack((detections,currentArray))


                '''
                Updating the tracker across multiple frames so that's why placed outside for loop
                '''
            resultTracker=tracker.update(detections)
            # print(resultTracker)

            '''
            data visualization part is always outside the ret
            '''
            ...
            for result in resultTracker:
                ...
                '''
                For checking if the vehicle is entering or leaving.
                '''
